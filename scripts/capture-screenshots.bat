...
title Image MultiModel Screenshot Capture

set "ROOT=%~dp0"
set "TESTS_DIR=%ROOT%tests"
set "BASE_URL=http://127.0.0.1:8288"
set "HEALTH=%BASE_URL%/api/health"

echo ============================================
echo   Image MultiModel - Auto Screenshot Capture
echo ============================================
echo.
...
echo [INFO] Starting Image MultiModel server ^(UI screenshots do not need GPU^) ...
start "Image MultiModel Server" /min "%PYTHON_CMD%" "%ROOT%bin\clean_launch.py"
...
echo ============================================
echo.
echo NOTE: The Image MultiModel server window may still be open ^(minimized^).
