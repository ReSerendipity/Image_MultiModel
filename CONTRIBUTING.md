...
感谢关注 Image MultiModel！本文件给出从克隆到合并的最短路径。
...
模型权重不随仓库分发，见 `LICENSE_COMPLIANCE.md`。
...
- `comfy_kernel/` 是 vendored 上游 ComfyUI 源码，禁止修改。
- `app/config.yaml` 是版本号单一事实来源（当前 v1.2.2）。
- 桌面壳（`desktop/src-tauri/`）版本号独立，见 `Cargo.toml`。
