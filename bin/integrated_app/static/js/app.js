...
document.getElementById('setOpen').addEventListener('click',openSet);
...
document.querySelectorAll('.sb-click').forEach(function(el){el.addEventListener('click',openStat);});
...
document.querySelectorAll('.ex-chip').forEach(function(c){c.addEventListener('click',function(){document.querySelectorAll('.ex-chip').forEach(function(x){x.classList.remove('active')});this.classList.add('active');posPrompt.value=this.textContent;updatePosMeta();});});
...
var zoom=100,compare=false,cur=0,_curViewer=null;
...
document.getElementById('vHead').addEventListener('pointerdown',function(e){dragging=true;dx=e.clientX-viewer.offsetLeft;dy=e.clientY-viewer.offsetTop;this.setPointerCapture(e.pointerId);});
document.getElementById('vHead').addEventListener('pointermove',function(e){if(!dragging)return;viewer.style.left=(e.clientX-dx)+'px';viewer.style.top=(e.clientY-dy)+'px';});
document.getElementById('vHead').addEventListener('pointerup',function(){dragging=false;});
/* ---------- 历史（抽屉内） ---------- */
...
document.querySelectorAll('#histRows tr').forEach(function(tr){tr.addEventListener('click',function(){document.querySelectorAll('#histRows tr').forEach(function(x){x.classList.remove('sel');});tr.classList.add('sel');document.getElementById('ddEngine').textContent=tr.dataset.e;document.getElementById('ddStatus').textContent=tr.dataset.s;document.getElementById('ddPrompt').textContent=tr.dataset.p;document.getElementById('ddDim').textContent=tr.dataset.d;document.getElementById('ddTime').textContent=tr.dataset.t;showHistDetail();});});
/* ---------- 预设（抽屉内） ---------- */
...
// 覆盖取消按钮
document.querySelector('.queue-pop .btn-sm').onclick=function(){cancelGenReal();qpop.classList.remove('show');};
  ...
  fetch(qs).then(function(r){return r.json();}).then(function(r){
    ...
    list.forEach(function(t){
      ...
      tr.innerHTML='<td><div class="th">'+(t.output_count>0?t.output_count+' 张':'—')+'</div></td>'+
        '<td class="pro">'+((t.prompt||'(无提示词)'))+'<br><span style="font-size:9px;color:var(--ink-faint)">'+engLabel(t.engine)+'</span></td>'+
...
function renderPresets(){
  ...
  fetch('/api/presets').then(function(r){return r.json();}).then(function(presets){
    pl.innerHTML='';
    if(!presets||!presets.length){pl.innerHTML='<p style="grid-column:1/-1;text-align:center;color:var(--ink-faint);font-size:11px;padding:14px">暂无预设，点击「＋ 新建预设」创建</p>';return;}
    presets.forEach(function(p){
      ...
      var c=document.createElement('div');c.className='p-card';
      c.innerHTML='<div class="nm">'+((p.name||'未命名').substring(0,20))+'</div>'+
...
renderGallery=function(filter){
  ...
  fetch('/api/outputs?page=1&page_size=50').then(function(r){return r.json();}).then(function(r){
    ...
    list.forEach(function(out){
      ...
      d.innerHTML='<span class="g-type">'+(TYPE_LABELS[out.output_type]||out.output_type||'输出')+'</span><div class="ph"><img src="/api/outputs/'+out.path+'" style="max-width:100%;max-height:200px;object-fit:contain" onerror="this.parentElement.textContent=\'加载失败\'"></div><div class="g-meta"><b>'+(out.prompt||'生成结果').substring(0,20)+'</b><span>'+engLabel(out.engine)+' · '+(out.created_at?String(out.created_at).substring(5,16):'')+'</span></div>';
      d.addEventListener('click',function(){openViewerReal(out);});
...
function openViewerReal(out){
  _curViewer=out;
  zoom=100;compare=false;
  vTitle.textContent=(out.prompt||'生成结果');
  vSeed.textContent=out.seed||'—';
  vMeta.textContent=engLabel(out.engine)+' · '+(TYPE_LABELS[out.output_type]||out.output_type||'');
  viewer.classList.add('show');renderImg();
}
...
openStat=function(){
  ...
  fetch('/api/health').then(function(r){return r.json();}).then(function(h){
    ...
    if(!infoBox){
      ...
      sd.querySelector('.drawer-body').prepend(infoBox);
    }
    infoBox.innerHTML=
      '<p><b>状态:</b> '+h.status+'</p>'+
...
function initAccent(){
  ...
  if(saved&&saved[0]){
    var LEGACY={'#fdba74':'#e8822a','#fb923c':'#cf6e1a','#7c5fd6':'#e8822a','#6b5bb8':'#cf6e1a','#f98a25':'#e8822a','#ef6d1c':'#cf6e1a'};
...
function loadRecent(){
  ...
  fetch('/api/outputs?page=1&page_size=9').then(function(r){return r.json();}).then(function(r){
    ...
    list.forEach(function(out){
      ...
      c.innerHTML='<div class="ph-img"><img src="/api/outputs/'+out.path+'" style="max-width:100%;max-height:220px;object-fit:contain;display:block;margin:0 auto" onerror="this.parentElement.textContent=\'加载失败\'"><div class="r-actions"><button class="btn btn-sm" type="button" onclick="window.open(\'/api/outputs/'+out.path+'\',\'_blank\')">下载</button><button class="btn btn-sm" type="button" onclick="window.open(\'/api/tasks/export?ids='+out.task_id+'\',\'_blank\')">ZIP</button><button class="btn btn-sm" type="button" onclick="redrawTask(\''+out.task_id+'\')">重绘</button></div></div><div class="r-meta"><b>'+((out.prompt||'生成结果').substring(0,18))+'</b><span>'+label+' · '+meta+'</span></div>';
      grid.appendChild(c);
    });
...
function renderQueue(){
  ...
  fetch('/api/tasks?page=1&page_size=30').then(function(r){return r.json();}).then(function(r){
    ...
    list.forEach(function(t){
      ...
      d.innerHTML='<div class="t"><b>'+(t.task_id||'').substring(0,12)+' · '+((t.prompt||'未命名').substring(0,16))+'</b><span>'+st+' · '+(t.output_count||0)+' 输出</span></div><span class="p">'+(t.status==='processing'?'…':'—')+'</span>';
      box.appendChild(d);
    });
