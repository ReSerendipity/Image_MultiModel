...
fn build_http_client(timeout: Duration) -> Result<reqwest::Client> {
    let mut builder = reqwest::Client::builder()
        .timeout(timeout)
        .user_agent("ImageMultiModel-Desktop");
...
async fn fetch_latest_release(
    ...
    let resp = client
        ...
        .header("User-Agent", "ImageMultiModel-Desktop")
        .send()
        .await
...
fn probe_writable(dir: &Path) -> bool {
    let probe = dir.join(".imgmulti_write_probe");
...
mod tests {
    ...
    fn sha256_known_vector() {
        let dir = std::env::temp_dir().join(format!("imgmulti_sha_{}", std::process::id()));
    ...
    fn extract_zip_roundtrip() {
        let dir = std::env::temp_dir().join(format!("imgmulti_zip_{}", std::process::id()));
        fs::create_dir_all(&dir).unwrap();
    ...
    fn swap_success_promotes_new_and_keeps_backup() {
        let root = std::env::temp_dir().join(format!("imgmulti_swap_ok_{}", std::process::id()));
    ...
    fn swap_rolls_back_when_promote_fails() {
        let root = std::env::temp_dir().join(format!("imgmulti_swap_fail_{}", std::process::id()));
        let _ = fs::remove_dir_all(&root);
        fs::create_dir_all(&root).unwrap();
        let app = mk_dir_with_marker(&root, "app", "OLD");
        let bak = root.join("app.bak");
    ...
    fn restore_backup_discards_new_and_recovers_old() {
        let root = std::env::temp_dir().join(format!("imgmulti_restore_{}", std::process::id()));
        let _ = fs::remove_dir_all(&root);
        fs::create_dir_all(&root).unwrap();
    ...
    fn rename_with_retry_moves_dir() {
        let root = std::env::temp_dir().join(format!("imgmulti_rename_{}", std::process::id()));
        let _ = fs::remove_dir_all(&root);
    ...
    fn preserve_heavy_dirs_copies_heavy_dirs_into_new() {
        ...
        let root = std::env::temp_dir().join(format!("imgmulti_preserve_{}", std::process::id()));
        let _ = fs::remove_dir_all(&root);
    ...
    fn preserve_heavy_dirs_respects_update_package_content() {
        ...
        let root = std::env::temp_dir().join(format!("imgmulti_preserve2_{}", std::process::id()));
        let _ = fs::remove_dir_all(&root);
        let app = root.join("app");
        let app_new = root.join("app.new");
