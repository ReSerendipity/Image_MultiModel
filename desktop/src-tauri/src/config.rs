//! 配置管理：应用共享目录（`%APPDATA%/ImageMultiModel`）、壳配置（更新设置/通知设置）
...
pub const APP_DATA_DIR_NAME: &str = "ImageMultiModel";

/// 返回 `%APPDATA%/ImageMultiModel`，必要时创建。配置缺失 `APPDATA` 时回退到 home 目录。
...
mod tests {
    ...
    fn default_config_roundtrip() {
        ...
        match back.update_source {
            UpdateSource::Github { owner, repo } => {
                ...
                assert_eq!(repo, "Image_MultiModel");
            }
    ...
    fn write_atomic_creates_file() {
        let dir = std::env::temp_dir().join(format!("imgmulti_cfg_test_{}", std::process::id()));
