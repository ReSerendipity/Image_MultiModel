...
//! └─ 退出 Image MultiModel
//! ```
//! 悬停提示：`Image MultiModel - 运行中` / `Image MultiModel - 正在处理`。
//!
...
const TOOLTIP_IDLE: &str = "Image MultiModel - 运行中";
const TOOLTIP_BUSY: &str = "Image MultiModel - 正在处理";
...
pub fn build_tray(app: &AppHandle) -> tauri::Result<()> {
    ...
    let quit = MenuItem::with_id(app, ID_QUIT, "退出 Image MultiModel", true, None::<&str>)?;

    let menu = Menu::with_items(app, &[&show, &hide, &sep1, &logs, &update, &sep2, &quit])?;

    let mut builder = TrayIconBuilder::with_id("imgmulti-tray")
        .menu(&menu)
