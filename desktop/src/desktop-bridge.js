/*
 * Image MultiModel 桌面壳前端桥接（desktop-bridge.js）
 * ------------------------------------------------------------------
...
(function () {
  ...
  if (window.__IMM_BRIDGE__) return; // 幂等：避免 init 脚本 + HTML 内联双份执行
  window.__IMM_BRIDGE__ = true;
  ...
  function toast(msg, type) {
    try {
      if (window.ImageMultiModel && typeof window.ImageMultiModel.toast === "function") {
        window.ImageMultiModel.toast(msg, type || "info");
        return;
      }
    } catch (e) {}
    // 后端 app.js 尚未就绪时兜底
  ...
  function onTaskEnd(status) {
    ...
    notifyTaskDone(
      ok ? "Image MultiModel 处理完成" : "Image MultiModel 处理结束",
  ...
  function hookSseCompletion() {
    ...
    var mo = new MutationObserver(function () {
      ...
      if (taskActive && !processing && done) {
        setBusy(false);
        notifyTaskDone("Image MultiModel 处理完成", "任务已完成，点击查看详情");
      }
    });
  ...
  function authorizeDir(dir) {
    if (!(window.ImageMultiModel && window.ImageMultiModel.api)) return Promise.resolve();
    return Promise.resolve()
      .then(function () { return window.ImageMultiModel.api.get("/api/system/settings"); })
      .then(function (data) {
        ...
        if (!dirs.some(function (d) { return norm(d) === norm(dir); })) {
          dirs.push(dir);
          return window.ImageMultiModel.api.post("/api/system/settings", { allowed_base_dirs: dirs });
        }
      })
  ...
  function ensureDialog() {
    ...
    dlgEl.innerHTML =
      ...
      '  <div style="padding:18px 22px;font-weight:600;font-size:17px;background:linear-gradient(135deg,#e94560,#f39c12);color:#fff">Image MultiModel 更新</div>' +
  ...
  function wireEvents() {
    ...
    listen("update-done", function (r) {
      notifyShell("Image MultiModel 更新完成", (r && r.message) || "应用已更新到最新版本");
      var body = document.getElementById("sdu-body");
      var actions = document.getElementById("sdu-actions");
