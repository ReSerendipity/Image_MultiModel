...
> 审计对象：FastAPI + 多引擎图像生成平台（默认 diffusers Z-Image-Turbo；旧版 native 引擎复用 vendored `comfy_kernel/`）
> 方法：静态扫描 + git 追踪面核查 + 配置校验器审查。未做动态渗透。

## 执行摘要（总体评级：中 / Medium）

无凭据入库、依赖锁定齐全、host 强制回环校验器为审计前已内置（先于本次审计）。主要暴露面集中在**旧版 native 引擎的 vendored ComfyUI 内核（GPL）** 与 **对外服务需显式配置认证**。已确认项见下，未发现硬编码密钥或命令注入。
...
### 4. 第三方组件
- **✓ 声明完整**：`THIRD_PARTY_NOTICES.md` 已列明 diffusers/Z-Image-Turbo（Apache-2.0）与弃用 ComfyUI 内核（GPL-3.0，仅旧版 native 引擎）义务。

## 待关注点位（非阻断）

| # | 级别 | 点位 | 建议 |
|---|---|---|---|
| 1 | Medium | 旧版 `z_image_turbo_native` 引擎若被启用，会进程内复用 GPL-3.0 的 `comfy_kernel/` | 默认已 deprecated 且本地禁用；保持禁用，分发时遵守 GPL 义务并附 `comfy_kernel/COMPLIANCE-README.md` |
