"""security/secret_key.py — 密钥管理模块（移植自 SeedVR2，命名对齐本项目）

职责：
- 服务端持久化密钥（HMAC-SHA256 回退签名用）：首次启动生成随机密钥持久化到
  ``data/.imm_secret``，后续重启复用；权限收紧（POSIX 0600 / Windows icacls）。
- 清单签名密钥对（Ed25519）：私钥仅存签发机/CI Secret（data/.manifest_signing_key），
  公钥内置 ``security/manifest_signing_public_key.pem`` 随代码分发（公开无害，
  仅用于验签，enforce 才能成立）。
- 任意文件 HMAC-SHA256 签名/校验（integrity_manifest.json 的 .sig 回退路径）。

安全策略（报告2 §三）：
- 密钥使用 secrets.token_bytes(32) 生成（256 位熵），hex 编码存储。
- 读取时自愈：每次 get_secret_key() 读取都调用 harden_secret_file_permissions()，
  历史部署的过宽权限在下次读取时自动修复（POSIX 0600 / Windows icacls 去继承）。
- 测试护栏：临时目录内跳过 icacls，避免破坏 pytest 临时目录回收（WinError 5）。
- icacls 在中文 Windows 输出 GBK 文本，显式 encoding + errors=replace 防解码崩溃。

使用方式:
    from app.integrated_app.security.secret_key import get_secret_key, sign_file, verify_file_signature

    key = get_secret_key()          # 32 字节随机密钥（bytes）
"""
...
# 默认密钥持久化路径（相对于项目根目录）
_DEFAULT_KEY_FILE = "data/.imm_secret"

# 密钥字节数
_KEY_BYTES = 32

# 环境变量覆盖（优先级最高，容器化/企业统一注入场景）
_SECRET_KEY_ENV = "IMAGE_MULTIMODEL_SECRET_KEY"
...
def get_secret_key(key_file: str | os.PathLike | None = None) -> bytes:
    ...
    优先级：环境变量 IMAGE_MULTIMODEL_SECRET_KEY → 密钥文件 → 生成并持久化。
    按密钥文件路径分键缓存（线程安全），同一文件路径重复调用返回同一密钥。

    Args:
        key_file: 密钥文件路径，为 None 时使用默认路径 data/.imm_secret。
...
def harden_secret_file_permissions(path: str | os.PathLike) -> bool:
    ...
    try:
        if os.name == "nt":
            ...
            # 临时目录内不跑 icacls：修改 ACL 会破坏 pytest 临时目录回收
            ...
            try:
                ...
                # icacls 在中文 Windows 输出 GBK 文本，而 PYTHONUTF8=1 环境的
                # text=True 会按 UTF-8 解码 → 读线程崩、stdout 静默丢失。
                # 显式 encoding + errors=replace 保证解码永不中断。
...
# ---------------------------------------------------------------------------
# 文件 HMAC-SHA256 签名 / 校验（清单 .sig 回退路径）
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 清单签名非对称化（Ed25519：构建机私钥签发 / 发布包内置公钥验证）
# ---------------------------------------------------------------------------
# 背景：HMAC 是对称签名，用户端要验签就必须持有密钥；密钥进包=公开可伪造，
# 不进包=发布版 enforce 会把所有用户锁死。非对称签名解决该矛盾：私钥只存
# 签发机（data/.manifest_signing_key，随包排除），公钥 PEM 内置到 security/
# 随代码分发（公开无害，仅用于验签）。
...
def generate_manifest_signing_keypair(force: bool = False) -> tuple[Path, Path]:
    ...
    # newline="\n"：Windows 文本模式默认会把 \n 转成 \r\n，导致仓库内公钥
    # PEM 行尾漂移（CRLF），与私钥派生公钥（LF）字节比对失配
    # （任务书 P0 实测：公钥一致性闸门 SHA256 不一致即此根因）。
    pub_path.write_text(pub_pem.decode("utf-8"), encoding="utf-8", newline="\n")
...
def sign_manifest_ed25519(
    ...
    try:
        from cryptography.hazmat.primitives import serialization
        from cryptography.hazmat.primitives.asymmetric import ed25519
    except Exception as e:  # noqa: BLE001 - 依赖不可用时降级
...
def verify_manifest_signature_ed25519(
    ...
    try:
        from cryptography.hazmat.primitives import serialization
        from cryptography.hazmat.primitives.asymmetric import ed25519
    except Exception as e:  # noqa: BLE001 - 依赖不可用时跳过公钥验证
