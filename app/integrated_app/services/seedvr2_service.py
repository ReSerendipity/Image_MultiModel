...
用户操作（仅需一次）：
    # 下载模型权重到 portable 目录
    huggingface-cli download ByteDance/SeedVR2 --local-dir model/SeedVR2/3b
    # 或使用 ModelScope（国内更快）
    modelscope download --model ByteDance/SeedVR2 --local_dir model/SeedVR2/3b
"""
...
try:
    ...
    from app.integrated_app.engines.seedvr2_engine import SeedVR2Engine, ImageInferenceConfig
...
class SeedVR2Service:
    ...
    async def ensure_loaded(
        ...
        # 模型路径：model/SeedVR2/{model_size}/
        if project_root:
            models_root = str(Path(project_root) / "model" / "SeedVR2" / model_size)
        else:
            models_root = str(Path("model") / "SeedVR2" / model_size)
