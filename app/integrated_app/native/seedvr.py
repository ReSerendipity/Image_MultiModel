...
#   - 模型目录：model/seedvr2/（portable 自包含，符合模型摆放约定）
...
_DEFAULT_MODELS_DIR = _PROJECT_ROOT / "model" / SEEDVR2_FOLDER
...
def resolve_weights(
    ...
    Args:
        models_dir: model/SEEDVR2 目录；为 None 用默认路径
        dit_model: DiT 权重文件名
        vae_model: VAE 权重文件名
...
def upscale_2x(
    ...
    Args:
        ...
        models_dir: 权重目录（默认 model/SEEDVR2）
        seedvr_source_dir: 自定义节点源码根目录
